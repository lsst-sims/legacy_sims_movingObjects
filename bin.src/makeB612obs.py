...
def setupArgs(parser=None):
    if parser is None:
        parser = argparse.ArgumentParser(description="Generate moving object detections, "
                                                     "with options appropriate for B612.")
    ...
    parser.add_argument("--footprint", type=str, default='rectangle',
    ...
    parser.add_argument("--xTol", type=float, default=10,
                        help="If using a rectangular footprint, this is the tolerance in the RA direction "
                             "(in degrees). Default is 10 degrees.")
    parser.add_argument("--yTol", type=float, default=5,
    ...
    parser.add_argument("--roughTol", type=float, default=20,
                        help="If using direct/exact ephemeris generation, this is the tolerance for the "
                             ...
                             "Default 20 degrees.")
    parser.add_argument("--obsType", type=str, default='linear',
    ...
    if args.obsType not in ('linear', 'direct'):
        ...
        # Check interpolation type.
    if args.obsType not in ('linear', 'direct'):
        print("Must choose linear or direct observation generation method (obsType).")
        exit()
