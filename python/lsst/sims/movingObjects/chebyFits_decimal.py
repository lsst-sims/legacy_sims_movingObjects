...
class ChebyFits(object):
    ...
    def __init__(self, orbitsObj, tStart, tSpan, timeScale='TAI',
        ...
        self.nDecimalExp = decimal.Decimal('%s' % round(10**-self.nDecimal, self.nDecimal))
        self.tStart = decimal.Decimal(tStart).quantize(self.nDecimalExp)
        self.tSpan = decimal.Decimal(tSpan).quantize(self.nDecimalExp)
    ...
    def _roundLength(self, length):
        ...
        """
        length = decimal.Decimal(length).quantize(self.nDecimalExp)
        length_in = length
        ...
        while ((self.tSpan % length) > tolerance) and (length > 0) and (counter < 10):
            ...
            length = (self.tSpan / int_factor).quantize(self.nDecimalExp)
            counter += 1
        if (self.tSpan % length) > tolerance:
            ...
            raise ValueError('Could not find a suitable length for the timespan (start %f, span %f), starting with %s'
                             % (self.tStart, self.tSpan, str(length_in)))
        return length
    ...
    def _testResiduals(self, length, cutoff=99):
        ...
        times = np.arange(float(self.tStart), float(self.tStart) + length + timestep / 2.0, timestep, dtype=float)
    ...
    def calcSegmentLength(self, length=None):
        ...
        if length is not None:
            ...
            self.length = length
            self.timestep = self.length / self.ngran
            return
        ...
        pos_resid, ratio = self._testResiduals(length)
        counter = 0
        # Use float version of length to avoid converting slope/y/residuals, etc.
        length = float(length)
        ...
        while pos_resid > self.skyTolerance and counter <= maxIterations:
            ...
            length = float(self._roundLength(length))
            pos_resid, ratio = self._testResiduals(length)
            counter += 1
        length = self._roundLength(length)
        self.length = length
    ...
    def calcSegments(self):
        ...
        for orbitObj, e in zip(self.orbitsObj, ephs):
            ...
            while tSegmentStart < self.tEnd:
                ...
                tSegmentEnd = tSegmentStart + self.length
    ...
    def _subdivideSegment(self, orbitObj, ephs):
        ...
        try:
            ...
            warnings.warn('Objid %s, segment %f to %f - error message %s'
                          % (orbitObj.orbits.objId.iloc[0], ephs['time'][0], ephs['time'][-1], ValueError.message))
